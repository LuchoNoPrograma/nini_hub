...
void main() {
  test(
    'activated calendar keeps legacy account/day rules with one aggregate query',
    () async {
      final selects = _SelectCounter();
      ...
      expect(selects.statements.single, contains('WITH token_max AS'));
      ...
      expect(usageDay.accounts.map((item) => item.profileId), [
        primary.id,
        secondary.id,
      ]);
      ...
      expect(resetDay.accounts.map((item) => item.resetCount), [1, 1]);
...
CliProfile _profile(String id, {String? displayName}) {
