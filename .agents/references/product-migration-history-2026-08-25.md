...
## Fix separado FIX-OPS-VIS-01: Heartbeat y Activity con feedback inmediato

- Estado global y HEAD observado al iniciar: `12/14`; Nini Hub `main` /
  `e44c2a1cae43f2dfddbc30dc8f2919db38b4ea6f`. El fix se clasifica
  `separate_fix`: no agrega un punto al roadmap ni cambia QA-01B/CUT-01.
- El diagnostico de solo lectura comprobo tres causas separadas: Ari ejecutaba
  correctamente el comando Heartbeat y solo exponia una ventana Codex de 30
  dias, pero la verificacion posterior no podia confirmar un ancla estable;
  `ProcessRunner` ya insertaba el log `running`, mientras Activity hacia una
  lectura puntual; y Magic persistia `TOKEN_EXPIRED`, aunque el card mostraba
  el estado generico `REQUIERE ACCESO`.
- Alcance aprobado el 2026-08-25: preservar la seleccion dinamica de ciclo y la
  ventana de 30 dias de Ari, distinguir comando enviado de ciclo aun no
  confirmado, publicar Activity desde `running` hasta finalizacion y mostrar
  `CREDENCIAL EXPIRADA` en Accounts. Se excluyeron cambios de politica
  Heartbeat, `nini-agents`, schema/generated, SQLite o credenciales reales,
  builds, instalacion y Git.
- Skills consumidas: `nini-hub-feature-integral`,
  `nini-hub-domain-application`, `nini-hub-data-desktop-integrations` y
  `nini-hub-presentation-flutter-desktop`.

### Antes/despues y contratos

- Antes, un Heartbeat exitoso pero no verificable informaba solo que Codex
  habia respondido, sin dejar claro que el comando si fue enviado ni cual ciclo
  intento comprobar. Ahora los resultados verificado y no verificado nombran
  primero el envio y expresan la duracion dinamica; Ari conserva 43200 minutos
  como `30 dias`, sin fallback semanal ni regla especial por nombre de perfil.
- Antes, Activity obtenia una foto de los ultimos 250 logs. Aunque
  `ProcessRunner` persistia la fila antes de iniciar el proceso, la vista podia
  verla recien despues de una recarga coordinada. `ActivityRepository` ahora
  expone `watchRecent`, Drift observa la misma query acotada y el controller
  mantiene una unica suscripcion correlacionada con su lifecycle. La vista
  publica comando y estado `running`, luego salida, exit code y estado terminal
  al actualizarse la misma fila.
- El stream conserva filtros, busqueda y seleccion locales; no recarga otros
  dominios. Una falla inicial mantiene el bloqueo de startup existente y una
  falla posterior conserva el ultimo snapshot con un error recuperable. La
  limpieza sigue siendo single-flight y focalizada.
- Accounts prioriza `credentialExpired` y `credentialInvalidated` sobre el
  estado generico de la consulta. Para `TOKEN_EXPIRED`, el card muestra
  `CREDENCIAL EXPIRADA` y pide volver a iniciar sesion; no se muta ni se lee la
  credencial.

### Archivos, persistencia y validacion

- Producto: puerto/caso de uso/repository/controller de Activity y composicion
  en `app/providers.dart`; mensaje de resultado en
  `features/heartbeat/application/heartbeat.dart`; y etiqueta/detalle en
  `features/accounts/presentation/accounts_view.dart`.
- Pruebas: Application/Data/controller/composicion/widget de Activity;
  Application y flujo visible de Heartbeat; caracterizacion de sincronizacion
  Usage adaptada al puerto reactivo; y card de Accounts en `widget_test.dart`.
- No cambiaron tablas, `schemaVersion`, migraciones, generated ni datos. El
  stream usa la misma query descendente con limite 250 y observa las escrituras
  existentes de `CommandLogs`; no agrega polling, procesos ni otra instancia de
  scheduler.
- `dart format` dejo limpios los 16 Dart del delta y `flutter analyze` focalizado
  termino sin issues. Pasaron 23/23 pruebas de Activity, 41/41 de Heartbeat,
  13/13 de Accounts, 29/29 de `widget_test.dart` y la guarda arquitectonica
  1/1.
- La caracterizacion Usage relacionada paso 4/5 casos; el unico assertion rojo
  es el ya documentado por FIX-UX-USAGE-01, que espera un overflow de marca de
  25 px que el layout vigente no produce. No se amplio este fix para alterar esa
  expectativa legacy.
- No se ejecuto suite global, build, instalacion, runtime Linux/Windows con
  procesos o perfiles reales, escritura SQLite real, renovacion de credenciales,
  stage, commit, push, merge ni release. El siguiente punto del roadmap sigue
  siendo QA-01B y cualquier ajuste del assertion legacy requiere alcance
  independiente.

## Fix separado FIX-SCRIPT-01: enlace vigente de compilacion e instalacion

- Estado global y HEAD observado al iniciar: `12/14`; Nini Hub `main` /
  `e44c2a1cae43f2dfddbc30dc8f2919db38b4ea6f`. El fix se clasifica
  `separate_fix` y no cambia QA-01B/CUT-01.
- Diagnostico: `scripts_personales/instacompilado.sh` resolvia
  `compilar_nini_hub.sh`, archivo inexistente en el filesystem vigente. Los dos
  scripts presentes eran `compilar.sh` e `instacompilado.sh`, ambos ejecutables
  con modo `775` y sintaxis Bash valida.
- Alcance aprobado el 2026-08-25: apuntar el instalador al compilador real,
  actualizar indice y bitacora con los nombres observados y validar de forma
  estatica. Se excluyeron build, instalacion, producto Flutter, SQLite, datos
  reales, credenciales y Git.
- `instacompilado.sh` ahora invoca `compilar.sh` desde el mismo `SCRIPT_DIR` y
  conserva build previo, busqueda del bundle, staging, backup, activacion,
  symlink y launchers existentes. No se alteraron rutas de instalacion ni
  comandos Flutter.
- `scripts_personales/` sigue ignorado y fuera del historial Git. El indice
  vigente refleja los dos nombres actuales; las menciones antiguas en fracciones
  historicas permanecen como evidencia del estado observado en esos cortes.
- Validacion ejecutada: permisos/existencia, `bash -n` de ambos scripts,
  resolucion estatica del target y `git diff --check`. No se ejecuto el launcher,
  por lo que no hubo build, staging, backup, instalacion ni escritura fuera del
  workspace.

## Fix separado FIX-OPS-NOISE-01: discovery unico y Activity operativa

- Estado global y HEAD observado al iniciar: `12/14`; Nini Hub `main` /
  `e44c2a1cae43f2dfddbc30dc8f2919db38b4ea6f`. El incidente se clasifico
  `separate_fix`; no cambia QA-01B/CUT-01.
- El diagnostico de solo lectura demostro un N+1 de arranque. Habia 17 perfiles
  elegibles para Heartbeat y una secuencia completa produjo 18 invocaciones
  `list` y 18 `tools`: un discovery general mas otro desde cada probe inicial.
  `ProfileDiscoveryService` ejecuta el par para cada discovery y
  `ProbeHeartbeat` redescubria el motor antes de consultar una sola cuenta.
- El mismo `ProcessRunner` registraba todas esas lecturas machine-safe como
  Activity de usuario, incluida la salida JSON completa. No era polling
  infinito ni un fallo de Ari, sino una rafaga determinista repetida con cada
  arranque y con probes posteriores.
- Alcance aprobado el 2026-08-25: eliminar el discovery por cuenta, impedir que
  lecturas internas `version/list/status/tools` creen Activity y conservar
  visibles mutaciones, launch y comandos Heartbeat. Se excluyeron politica de
  ciclos, `nini-agents`, schema/generated, borrado o escritura de SQLite real,
  credenciales, build, instalacion y Git.
- Skills consumidas: `nini-hub-diagnostico-incidentes`,
  `nini-hub-feature-integral`, `nini-hub-domain-application` y
  `nini-hub-data-desktop-integrations`.

### Contratos y antes/despues

- `ProcessRunner.run()` incorpora `recordActivity`, activo por defecto. Con
  `false` conserva executable/args, environment, timeout, cancelacion,
  sanitizacion y resultado, pero no inserta ni finaliza una fila
  `CommandLogs`. `NiniAgentsReadClient` usa este modo solo en `version`, `list`,
  `status` y `tools`; create/rename/delete mantienen el registro normal.
- `ProbeHeartbeat` reemplaza `ProfileDiscovery` por el puerto existente
  `ProfileRepository.findById`. La composicion usa `DriftProfileRepository`, de
  modo que cada probe obtiene el perfil ya sincronizado con una consulta local
  y no vuelve a ejecutar `nini-agents list/tools`. El Heartbeat manual conserva
  el rediscovery fresco previo a una accion explicita.
- Antes, 17 cuentas producian 36 procesos de discovery visibles durante un
  arranque. Ahora el arranque mantiene un unico discovery necesario y sus
  lecturas internas no aparecen en Activity; los comandos que realmente
  interesan al usuario siguen publicando `running` y su finalizacion en vivo.
- No se eliminaron filas historicas. Las consultas anteriores permanecen hasta
  que el usuario limpie Activity o salgan del limite de 250; la version nueva no
  genera nuevas filas de ese tipo.

### Archivos, persistencia y validacion

- Producto: `lib/core/process/process_runner.dart`,
  `lib/core/process/nini_agents_read_client.dart`,
  `lib/features/heartbeat/application/heartbeat.dart` y composicion en
  `lib/app/providers.dart`. Se adaptaron doubles de proceso en pruebas Profiles,
  Heartbeat y widget sin cambiar sus contratos funcionales.
- No cambiaron tablas, migraciones, `schemaVersion`, generated ni datos reales.
  El diagnostico SQLite fue estrictamente read-only y no expuso contenido
  sensible en esta bitacora.
- `dart format` dejo limpios los Dart tocados y `flutter analyze` focalizado
  termino sin issues. Pasaron 39/39 regresiones de ProcessRunner, cliente Nini,
  discovery, lifecycle y Heartbeat Application; ademas 79/79 de Heartbeat
  completo, composiciones Accounts/Activity/Settings, widgets y guarda
  arquitectonica.
- No se ejecuto suite global, build, instalacion, runtime de la aplicacion nueva,
  procesos contra perfiles reales, escritura SQLite, limpieza de Activity,
  stage, commit, push, merge ni release. La comprobacion manual posterior debe
  reinstalar la version y confirmar que un arranque no agrega filas
  `Consultar Nini Agents: list/tools`.

## Fix separado FIX-UX-QUOTA-01: contador reactivo y anclas verificables

- Estado global y HEAD observado al iniciar: `12/14`; Nini Hub `main` /
  `e44c2a1cae43f2dfddbc30dc8f2919db38b4ea6f`. El incidente se clasifica
  `separate_fix`; no agrega un punto al roadmap ni cambia QA-01B/CUT-01.
- El diagnostico de solo lectura comprobo dos causas. `formatTimeRemaining`
  calculaba correctamente contra `DateTime.now`, pero `_QuotaBar` era estatica
  y Accounts no poseia reloj ni ticker que provocara nuevos builds. Ademas,
  cualquier `resetsAt` del app-server se presentaba como `Reinicia en`, aunque
  algunas ventanas sin actividad devolvian siempre `observedAt + duration` y
  desplazaban su supuesta hora de reinicio con cada consulta.
- La evidencia comparativa distinguio ambos comportamientos: una ventana de 5
  horas movio su ancla en el mismo intervalo que la observacion y era una
  proyeccion flotante; otra con uso informado no siguio ese movimiento. La
  consulta masiva no habia omitido perfiles: el desfase inicial correspondia a
  su posicion dentro de la cola acotada.
- Alcance aprobado el 2026-08-25: introducir una politica compartida de anclas,
  conservar dos lecturas exitosas por perfil, rotarlas durante la proyeccion
  incremental y usar un unico reloj reactivo para los cards. Se excluyeron
  Heartbeat automatico, cambios de schema/generated, datos o credenciales
  reales, `nini-agents`, builds, instalacion y operaciones Git.
- Skills consumidas: `nini-hub-feature-integral`,
  `nini-hub-domain-application`, `nini-hub-data-desktop-integrations` y
  `nini-hub-presentation-flutter-desktop`.
...
- Antes, el texto era una foto del instante de build y permanecia inmovil hasta
  otra accion visual. Ahora `accountsQuotaClockProvider` emite inmediatamente y
  cada minuto; Accounts observa ese unico stream auto-descartable y pasa la
  misma marca temporal a todos los cards. No existe un timer por cuenta.
- `QuotaResetAnchorPolicy` centraliza tolerancia de proyeccion de 3 minutos,
  estabilidad de 2 segundos y muestra minima de deriva de 20 segundos. Dos
  lecturas compatibles que avanzan observacion y ancla al mismo ritmo se
  clasifican `estimated`; un ancla estable o uso positivo se clasifica
  `confirmed`, salvo que la deriva ya demuestre una proyeccion flotante. Sin
  evidencia suficiente se conserva la etiqueta prudente `Estimado en`.
- `HeartbeatPolicy` consume las mismas primitivas de estabilidad y proyeccion,
  pero mantiene su umbral virgen de 1 %, decisiones, retries y verificacion
  existentes. Accounts muestra `Reinicia en` solo para `confirmed`; ambas
  etiquetas avanzan con el reloj y su tooltip expone la hora local absoluta y
  el nivel de confianza.
- `AccountUsageCheck` conserva `completedAt` como hora preferida de observacion.
  El modelo agrega la lectura exitosa anterior y empareja ventanas por
  `limitId/windowType` normalizados. Un snapshot exitoso mas nuevo rota la
  lectura previa; uno parcial compara la ventana visible contra el ultimo exito
  y una respuesta antigua no reemplaza estado nuevo.

### Persistencia, archivos y validacion

- `DriftAccountRepository` cambia el ranking exitoso de uno a dos registros por
  perfil y carga sus ventanas en el mismo select por IDs. El limite sigue siendo
  siete selects sin importar la cantidad de cuentas; no cambia tablas,
  `schemaVersion`, migraciones, generated, escrituras ni retencion historica.
- Producto: nueva policy en `features/usage/domain`, modelo/mapper/repository de
  Accounts, proyeccion `ProjectUsageAccount`, policy de Heartbeat, reloj y vista
  de Accounts. Pruebas focalizadas cubren deriva flotante, ancla estable, uso
  redondeado, fallback, dos filas Drift, rotacion incremental, tick compartido,
  avance visible y cleanup del provider en composicion externa.
- `dart format` dejo limpios los Dart del delta y `flutter analyze` focalizado
  termino sin issues. Pasaron 127/127 pruebas de Accounts, Heartbeat, policy de
  anclas, proyeccion Usage, widgets y guarda arquitectonica.
- No se ejecuto suite global, build, instalacion, runtime manual Linux/Windows,
  consultas nuevas contra perfiles reales, escritura SQLite real, stage,
  commit, push, merge ni release. La comprobacion manual posterior debe abrir
  Accounts, dejar pasar al menos un minuto y confirmar la diferencia visual
  entre una ventana anclada y una estimada.
